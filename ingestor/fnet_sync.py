...
def _parse_competencia_ref(value) -> dt.date | None:
    """dataReferencia do FNET: '05/2026' ou '10/07/2026' -> primeiro dia do mês."""
    text = _as_str(value)
    if not text:
        return None
...
def sync_documentos(
    ...
    *,
    ...
    filtro do FNET é por data, e o upsert absorve repetidos).
    """
    fonte = f"fnet_documentos:tf={tipo_fundo or 'all'}:cat={id_categoria or 'all'}"
